...
import pandas as pd
import glob
from utils.file_processing import map_proteins_to_reference_ligands
from utils.pdb_utils import extract_ligand_from_complex
from utils.smiles_processing import smiles_to_pdb
from utils.docking_functions import run_gnina_site_specific_docking
from utils.file_processing import update_results_xlsx, process_docking_results


def render_site_specific_docking_inputs():
    """Render the site-specific docking input interface"""
    st.header("Step 1: Input Parameters - Site-specific Docking with Reference Ligand")
    ...
    # Reference ligands input
    st.session_state.reference_ligands_dir = st.text_input(
        "Reference Ligands Directory Path:",
        value=st.session_state.reference_ligands_dir,
        help="Path to the directory containing reference ligand PDB files (protein-ligand complexes)"
    )
    ...
    if st.button("Proceed to Parameters"):
        ...
        if not os.path.isdir(st.session_state.reference_ligands_dir):
            errors.append("Reference ligands directory does not exist.")
        ...
        if errors:
            ...
            # Map proteins to reference ligands
            st.info("Mapping proteins to reference ligands based on sequence...")
            mapping = map_proteins_to_reference_ligands(st.session_state.proteins_dir, st.session_state.reference_ligands_dir)
            st.session_state.protein_ref_mapping = mapping
            
            if not mapping:
                st.error("No protein-reference ligand mappings found. Please check your input files.")
            else:
                st.success(f"Successfully mapped {len(mapping)} proteins to reference ligands")
                st.session_state.stage = 2.1  # Parameters for site-specific docking
                st.rerun()

def render_site_specific_docking_parameters():
    """Render the site-specific docking parameters interface"""
    st.header("Step 2: Docking Parameters - Site-specific Docking")
    ...
    with col2:
        ...
        # Add the new autobox_add parameter
        st.session_state.autobox_add = st.number_input(
            "Bounding Box Padding (Å):",
            min_value=0.0,
            max_value=50.0,
            value=4.0,
            step=0.5,
            help="The bounding box defines the search space for docking.\nYou can adjust the padding around the reference binding pocket to increase or decrease the box size."
        )
    ...
    with col3:
        if st.button("Back to Inputs"):
            st.session_state.stage = 1.2
            st.rerun()
    
    with col4:
        if st.button("Run Screening"):
            st.session_state.stage = 3.1
            st.rerun()

def render_site_specific_docking_execution():
    """Render the site-specific docking execution interface"""
    st.header("Step 3: Running Virtual Screening - Site-specific Docking")
    
    # Create a temporary directory for extracted ligands
    extracted_ligands_dir = os.path.join(st.session_state.output_dir, "extracted_reference_ligands")
    os.makedirs(extracted_ligands_dir, exist_ok=True)
    st.session_state.extracted_ligands_dir = extracted_ligands_dir
    
    # Extract ligands from reference complexes
    st.info("Extracting ligands from reference complexes...")
    
    # Get all unique reference complexes from the mapping
    unique_ref_files = set(st.session_state.protein_ref_mapping.values())
    extracted_ligands_mapping = {}
    
    progress_bar_extract = st.progress(0)
    status_text_extract = st.empty()
    
    for i, ref_file in enumerate(unique_ref_files):
        ref_name = os.path.basename(ref_file).replace('.pdb', '')
        output_path = os.path.join(extracted_ligands_dir, f"{ref_name}_ligand.pdb")
        
        status_text_extract.text(f"Extracting ligand from {ref_name}...")
        if extract_ligand_from_complex(ref_file, output_path):
            extracted_ligands_mapping[ref_file] = output_path
            st.session_state.docking_log.append(f"✓ Extracted ligand from {ref_name}")
        else:
            st.session_state.error_log.append(f"Failed to extract ligand from {ref_name}")
            st.session_state.docking_log.append(f"✗ Failed to extract ligand from {ref_name}")
        
        progress_bar_extract.progress((i + 1) / len(unique_ref_files))
    
    progress_bar_extract.empty()
    status_text_extract.text("Ligand extraction completed!")
    
    # Check if we successfully extracted any ligands
    if not extracted_ligands_mapping:
        st.error("No ligands were successfully extracted from reference complexes.")
        st.stop()
    
    # Get protein files
    protein_files = list(st.session_state.protein_ref_mapping.keys())
    if not protein_files:
        st.error("No protein files found in the mapping.")
        st.stop()
    ...
    for lig_idx, (ligand_file, ligand_name) in enumerate(zip(ligand_files, ligand_names)):
        for prot_idx, protein_file in enumerate(protein_files):
            protein_name = os.path.basename(protein_file).replace('.pdb', '')
            ref_complex_file = st.session_state.protein_ref_mapping[protein_file]
            
            # Get the extracted ligand file for this complex
            if ref_complex_file in extracted_ligands_mapping:
                ref_ligand_file = extracted_ligands_mapping[ref_complex_file]
            else:
                st.session_state.error_log.append(f"No extracted ligand found for {protein_name}")
                st.session_state.docking_log.append(f"✗ Skipping {protein_name}: no extracted ligand")
                continue
            
            # Add to log
            log_entry = f"Docking {ligand_name} with {protein_name} using reference {os.path.basename(ref_ligand_file)}"
            ...
            # Run docking
            affinity, pose = run_gnina_site_specific_docking(
            protein_file, 
            ligand_file, 
            ref_ligand_file,
            output_path, 
